...
export default function App() {
  ...
  const handleBulkArchive = async () => {
    ...
    const archiveToastId = addToast({
      type: 'loading',
      title: 'Bulk Archive Initiated',
      description: `Compressing ${selectedFiles.size} log files into an encrypted vault...`,
      duration: 0
    });
    ...
    try {
      ...
      for (const id of selectedFiles) {
        ...
        // Use export for Google Docs/Sheets
        if (file.mimeType.includes('google-apps')) {
          const exportMime = file.mimeType === 'application/vnd.google-apps.spreadsheet' 
            ? 'text/csv' 
            : 'application/pdf';
          url = `https://www.googleapis.com/drive/v3/files/${id}/export?mimeType=${exportMime}`;
        }
        
        const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
        if (res.ok) {
          const blob = await res.blob();
          const ext = file.mimeType === 'application/vnd.google-apps.spreadsheet' ? '.csv' : (file.mimeType.includes('google-apps') ? '.pdf' : '');
          zip.file(`${file.name}${ext}`, blob);
          processed++;
          
          updateToast(archiveToastId, {
            description: `Archiving [${processed}/${selectedFiles.size}]: ${file.name}`
          });
        }
      }
      
      const content = await zip.generateAsync({ type: 'blob', compression: 'DEFLATE' });
      saveAs(content, `TITAN_ARCHIVE_${new Date().getTime()}.zip`);
      
      updateToast(archiveToastId, {
        type: 'success',
        title: 'Archive Secured',
        description: `Successfully compressed ${processed} files into encrypted archive.`,
        duration: 5000
      });
      ...
      updateToast(archiveToastId, {
        ...
        description: err instanceof Error ? err.message : 'An error occurred during compression.',
        duration: 5000
      });
    } finally {
                ...
                <div className="flex items-center gap-2">
                  {selectedFiles.size > 0 && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-900/30 text-emerald-400 hover:bg-emerald-900/50 hover:text-emerald-300 border border-emerald-800 rounded text-xs font-bold tracking-widest uppercase transition-colors disabled:opacity-50"
                    >
                      <FileDown className="w-3.5 h-3.5" />
                      {isArchiving ? 'ARCHIVING...' : `BULK ARCHIVE (${selectedFiles.size})`}
                    </button>
                  )}
                  <button
                      ...
                      <li key={file.id} className="p-3 flex items-center justify-between hover:bg-[#1a1a1a] transition-colors">
                        <div className="flex items-center gap-3 overflow-hidden">
                          <input 
                            ...
                            className="w-4 h-4 rounded border-gray-700 text-emerald-500 focus:ring-emerald-500 bg-gray-900 shrink-0"
                          />
                          <span className="text-sm font-medium text-gray-300 truncate">{file.name}</span>
                        </div>
                        <span className="text-[10px] text-gray-500 font-mono bg-gray-800 px-1.5 py-0.5 rounded uppercase ml-4 shrink-0">
